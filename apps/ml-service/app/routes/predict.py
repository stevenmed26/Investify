...
def build_rule_based_prediction(symbol: str, horizon_days: int) -> PredictResponse | None:
    logger.info("[predict-route] using rule-based fallback symbol=%s horizon_days=%d", symbol, horizon_days)
    ...
    if not row:
        logger.warning(
            "[predict-route] no technical features found symbol=%s - returning no-data response (run backfill first)",
            symbol,
        )
        return None
...
def predict(
    ...
    trained = predict_with_trained_model(symbol, payload.horizon_days)
    if trained is not None:
        return trained

    rule_based = build_rule_based_prediction(symbol, payload.horizon_days)
